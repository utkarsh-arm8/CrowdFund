...
import {
  encode
} from "./chunk-GUSRGUBP.js";
import {
  getBytecode
} from "./chunk-U4XESRYZ.js";
import {
  formatBlock,
  hexToBytes,
  readContract
...
async function download(options) {
  ...
  if (options.uri.startsWith("ar://")) {
    const { resolveArweaveScheme } = await import("./arweave-JJJPKA2H.js");
...
async function resolvePluginPatternAddresses(contract) {
  try {
    const { readContract: readContract2 } = await import("./read-contract-6NHSTYCV.js");
...
async function estimateGas(options) {
  ...
  const promise = (async () => {
    ...
    const { encode: encode2 } = await import("./encode-FDJ26HTF.js");
    ...
    const [{ getRpcClient: getRpcClient2 }, { eth_estimateGas }] = await Promise.all([
      import("./rpc-UEHO5PRY.js"),
      import("./eth_estimateGas-XMES4RBM.js")
    ]);
...
async function toSerializableTransaction(options) {
  ...
  let [data, nonce, gas, feeData, to, accessList, value] = await Promise.all([
    ...
    (async () => {
      ...
      return from ? await import("./eth_getTransactionCount-KPO4ZOEH.js").then(({ eth_getTransactionCount }) => eth_getTransactionCount(rpcRequest, {
        address: from,
        blockTag: "pending"
      })) : void 0;
    })(),
...
//# sourceMappingURL=chunk-5Y4RQ6D5.js.map
