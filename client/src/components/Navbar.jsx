...
import { logo, menu } from '../assets'; // Importing logo.svg from assets
...
const Navbar = () => {
  ...
  const { connect, address, userProfile } = useStateContext(); // Assuming filterCampaigns exists in context
  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      ...
      <div className="lg:flex-1 flex flex-row justify-start py-2">
        <img src={logo} alt="Logo" className="w-[100px] h-[40px] object-contain" /> {/* Reduced height of the logo */}
      </div>
...
{/* 
        <Link to="/profile">
          <div className="w-[52px] h-[52px] rounded-full bg-[#2c2f32] flex justify-center items-center cursor-pointer overflow-hidden">
            {userProfile?.picture ? (
              ...
              <div className="w-full h-full bg-gray-400 flex justify-center items-center text-white">
                <span>{getInitials(userProfile?.displayName || userProfile?.email)}</span>
              </div>
            )}
          </div>
        </Link> */}
      ...
      <div className="sm:hidden flex justify-between items-center relative">
        <div className="w-[40px] h-[40px] rounded-[10px] bg-[#2c2f32] flex justify-center items-center cursor-pointer">
          <img src={logo} alt="user" className="w-[50%] h-[50%] object-contain" /> {/* Adjusted smaller logo */}
        </div>
